...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      remove: '移除',
      removeTitle: '移除这一行',
      copilotGroupTitle: 'GitHub Copilot 视觉后端',
      copilotGroupDesc: '使用你的 GitHub Copilot 订阅作为视觉模型：登录授权一次后自动调用（Responses API，无需 API Key）。失败时自动降级到下方链路。',
      copilotEnable: '启用 GitHub Copilot 作为视觉后端',
      copilotModel: 'Copilot 模型',
      copilotModelHint: '实测性价比（同一探针，AIU 单次成本）：gpt-5.6-luna 0.0018（最便宜）< gpt-5.4-mini 0.017 < gpt-5.6-terra 0.018 < gpt-5.4 0.09 < grok-4.6 0.42；gemini-3.x / kimi-k3 走另一端点，约 0.07–0.17，均比 luna 贵',
      copilotMaxTokens: '最大输出 tokens',
      copilotLoginStatus: 'GitHub 登录',
      copilotLoggedIn: '已登录',
      copilotNotLoggedIn: '未登录',
      copilotLoginButton: '开始登录',
      copilotLoginStep1: '1. 打开',
      copilotLoginStep2: '2. 输入代码',
      copilotLoginWaiting: '等待浏览器授权…（约 15 分钟内有效）',
      copilotLoginOk: '登录成功 ✓ 授权令牌已保存',
      copilotLoginFailed: '登录失败：',
      copilotTest: '测试连接',
      copilotTesting: '测试中…',
      copilotTestOk: '连接正常（',
      copilotTestFail: '连接失败：',
    ...
    const en = {
      ...
      copilotGroupTitle: 'GitHub Copilot vision backend',
      copilotGroupDesc: 'Use your GitHub Copilot subscription as the vision model: authorize once through GitHub login, then calls flow over the Copilot Responses API (no API key). Failures automatically fall through to the chain below.',
      copilotEnable: 'Enable GitHub Copilot as a vision backend',
      copilotModel: 'Copilot model',
      copilotModelHint: 'Measured value (same probe, AIU per call): gpt-5.6-luna 0.0018 (cheapest) < gpt-5.4-mini 0.017 < gpt-5.6-terra 0.018 < gpt-5.4 0.09 < grok-4.6 0.42; gemini-3.x / kimi-k3 use the other endpoint at ~0.07–0.17, all pricier than luna',
      copilotMaxTokens: 'Max output tokens',
      copilotLoginStatus: 'GitHub login',
      copilotLoggedIn: 'Logged in',
      copilotNotLoggedIn: 'Not logged in',
      copilotLoginButton: 'Start login',
      copilotLoginStep1: '1. Open',
      copilotLoginStep2: '2. Enter code',
      copilotLoginWaiting: 'Waiting for browser authorization… (expires in ~15 min)',
      copilotLoginOk: 'Login succeeded ✓ token saved',
      copilotLoginFailed: 'Login failed: ',
      copilotTest: 'Test connection',
      copilotTesting: 'Testing…',
      copilotTestOk: 'Connection OK (',
      copilotTestFail: 'Connection failed: ',
    ...
    function VisionRouterCard(props) {
      ...
      const [copilotStatus, setCopilotStatus] = useState({ status: 'idle', result: undefined })
      const [copilotLoginState, setCopilotLoginState] = useState({
        status: 'idle', flowId: undefined, verificationUri: undefined, userCode: undefined, error: undefined,
      })
      const [copilotTestState, setCopilotTestState] = useState({ status: 'idle', result: undefined })
      ...
      const format = (key) => {
        ...
        if (key === 'httpProviders') {
          // Editor value for the Copilot section: the full array (the section
          // keeps any non-copilot entries it finds).
      ...
      const parse = (key, text) => {
        ...
        if (key === 'httpProviders') {
          // Structured rows from the Copilot section: copilot entries are
          // normalized (model required), other entries pass through when they
          // carry name/baseURL/model. An empty result clears the field, so the
          // built-in OVH fallback chain takes over again.
          ...
          for (const row of text) {
            ...
            if (row.copilot === true) {
              ...
              const maxTokens = Number.isFinite(row.maxTokens) && row.maxTokens > 0 ? row.maxTokens : 4096
              list.push({
                name: typeof row.name === 'string' && row.name !== '' ? row.name : 'copilot',
                baseURL:
              ...
              list.push({
                ...
                baseURL,
                model,
                ...(Number.isFinite(row.maxTokens) && row.maxTokens > 0 ? { maxTokens: row.maxTokens } : {}),
              })
            }
          }
          return list.length > 0 ? { value: list } : { clear: true }
      ...
      const loadCopilotStatus = async () => {
        try {
          const response = await fetch('/_dsh/vision-router/copilot-status', { cache: 'no-store' })
          const result = await response.json().catch(() => undefined)
          setCopilotStatus({ status: 'done', result })
        } catch (error) {
          setCopilotStatus({
            status: 'done',
      ...
      const pollCopilotLogin = async (flowId) => {
        for (let attempt = 0; attempt < 240; attempt++) {
          await new Promise((resolve) => setTimeout(resolve, 4000))
          try {
            const response = await fetch(
              '/_dsh/vision-router/copilot-login?flowId=' + encodeURIComponent(flowId),
              { cache: 'no-store' },
            )
            const result = await response.json().catch(() => undefined)
            if (!result || result.ok !== true) continue
            if (result.status === 'authorized') {
              setCopilotLoginState((prev) => ({ ...prev, status: 'done' }))
              loadCopilotStatus()
              return
            }
            if (result.status === 'failed') {
              setCopilotLoginState((prev) => ({ ...prev, status: 'failed', error: result.error }))
              return
            }
          } catch {
        ...
        setCopilotLoginState((prev) => ({ ...prev, status: 'failed', error: 'timed out' }))
      }

      const startCopilotLogin = async () => {
        if (copilotLoginState.status === 'starting' || copilotLoginState.status === 'waiting') return
        setCopilotLoginState({ status: 'starting', flowId: undefined, verificationUri: undefined, userCode: undefined, error: undefined })
        try {
          const response = await fetch('/_dsh/vision-router/copilot-login', { method: 'POST', cache: 'no-store' })
          const result = await response.json().catch(() => undefined)
          ...
          void pollCopilotLogin(result.flowId)
        } catch (error) {
          setCopilotLoginState({
            status: 'failed',
            error: error && error.message ? error.message : String(error),
      ...
      const runCopilotTest = async () => {
        if (copilotTestState.status === 'running') return
        setCopilotTestState({ status: 'running', result: undefined })
        try {
          const response = await fetch('/_dsh/vision-router/copilot-test', { method: 'POST', cache: 'no-store' })
          const result = await response.json().catch(() => undefined)
          setCopilotTestState({
            status: 'done',
            result: result || { ok: false, error: 'HTTP ' + response.status },
          })
        } catch (error) {
          setCopilotTestState({
            status: 'done',
      ...
      const copilotProvidersOf = () => {
        const value = format('httpProviders')
        return Array.isArray(value) ? value : []
      }
      const copilotEntry = () => copilotProvidersOf().find((provider) => provider && provider.copilot === true)
      const copilotOthers = () => copilotProvidersOf().filter((provider) => !(provider && provider.copilot === true))
      const setCopilotDraft = (patch) => {
        const base = copilotEntry() || {}
        const entry = {
          name: 'copilot',
        ...
        setDraft('httpProviders', [...copilotOthers(), entry])
      }

      const copilotPanel = () => {
        const entry = copilotEntry()
        const enabled = entry !== undefined
        const login = copilotLoginState
        const status = copilotStatus.result
        const loggedIn = status && (status.method === 'env' || status.method === 'oauth-file')
        const test = copilotTestState
        return h('div', { className: 'vr-group', key: 'copilot-group' },
          h('p', { className: 'vr-group-title' }, t('copilotGroupTitle')),
          h('p', { className: 'vr-hint' }, t('copilotGroupDesc')),
          h('div', { className: 'vr-field' },
            h('div', { className: 'vr-field-head' },
              h('div', { className: 'vr-toggle' },
                h('span', { className: 'vr-label' }, t('copilotEnable')),
                h('input', {
                  type: 'checkbox', className: 'vr-check', checked: enabled, disabled: editBlocked,
                  onChange: (event) => {
                    if (event.target.checked) setCopilotDraft({})
                    else setDraft('httpProviders', copilotOthers())
                  },
                }),
              ),
          ...
          enabled
            ? h('div', { className: 'vr-field' },
                h('div', { className: 'vr-field-head' },
                  h('label', { className: 'vr-label' }, t('copilotModel')),
                ),
                h('input', {
                  className: 'vr-input', value: entry.model ?? '', disabled: editBlocked,
                  placeholder: 'gpt-5.6-luna',
                  onChange: (event) => setCopilotDraft({ model: event.target.value }),
                }),
                h('p', { className: 'vr-hint' }, t('copilotModelHint')),
              )
            : null,
          enabled
            ? h('div', { className: 'vr-field' },
                h('div', { className: 'vr-field-head' },
                  h('label', { className: 'vr-label' }, t('copilotMaxTokens')),
                ),
                h('input', {
                  type: 'number', className: 'vr-input', min: 1,
                  value: entry.maxTokens ?? 4096, disabled: editBlocked,
                  onChange: (event) => setCopilotDraft({ maxTokens: Number(event.target.value) }),
                }),
              )
            : null,
          h('div', { className: 'vr-field' },
            h('div', { className: 'vr-field-head' },
              h('span', { className: 'vr-label' }, t('copilotLoginStatus')),
              h('span', { className: 'vr-badge' }, loggedIn ? t('copilotLoggedIn') : t('copilotNotLoggedIn')),
            ),
            login.status === 'waiting'
              ? h('div', {},
                  h('p', { className: 'vr-hint' }, t('copilotLoginStep1') + ' ' + login.verificationUri),
                  h('p', { className: 'vr-hint' }, t('copilotLoginStep2') + ' ' + login.userCode),
                  h('p', { className: 'vr-hint' }, t('copilotLoginWaiting')),
                )
              : login.status === 'done'
                ? h('p', { className: 'vr-hint' }, t('copilotLoginOk'))
                : login.status === 'failed'
                  ? h('p', { className: 'vr-invalid' }, t('copilotLoginFailed') + (login.error || ''))
                  : null,
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', paddingTop: 4 } },
              h('button', {
                type: 'button', className: 'vr-btn',
                disabled: editBlocked || login.status === 'starting' || login.status === 'waiting',
                onClick: startCopilotLogin,
              }, t('copilotLoginButton')),
              h('button', {
                type: 'button', className: 'vr-btn',
                disabled: test.status === 'running' || !loggedIn,
                onClick: runCopilotTest,
              }, test.status === 'running' ? t('copilotTesting') : t('copilotTest')),
              test.status === 'done' && test.result
                ? h('p', {
                    className: test.result.ok ? 'vr-hint' : 'vr-invalid',
                    style: { margin: 0, minWidth: 0, flex: 1 },
                  },
                    test.result.ok
                      ? t('copilotTestOk') + test.result.latencyMs + 'ms, ' + test.result.answer + ')'
                      : t('copilotTestFail') + (test.result.error || ''),
                  )
                : null,
            ),
          ),
